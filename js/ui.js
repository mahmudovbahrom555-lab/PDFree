...
import { id, show, hide, setText, esc } from './utils.js';
...
export function setProgress(percent, label = '') {
  ...
  if (label) {
    lbl.innerHTML = `<span>${esc(label)}</span><span style="color:var(--green);font-size:12px;opacity:0.8">🔒 Processed locally</span>`;
  }
}
