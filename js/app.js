...
import { TOOLS, APP_VERSION, DONATE_URL, USAGE_KEY, DONATE_DELAY_MS,
...
function showTool(tool, pushHistory = true) {
  ...
  if (pushHistory) {
    const urlPath = tool.includes('pdf') ? `/${tool}/` : `/${tool}-pdf/`;
    history.pushState({ tool }, t.title, urlPath);
  }
  resetState();
...
function initEvents() {
  ...
  // Handle SPA routing on real anchor links
  document.querySelectorAll('a[data-tool]').forEach(el => {
